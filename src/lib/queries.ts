// ============================================================================
// queries.ts — COMPLETE fixed version with ALL functions used across pages
// ============================================================================
...
export async function fetchSimilarCars(car: Car, limit = 4): Promise<Car[]> {
  const { data } = await supabase
    .from('cars').select('*').neq('id', car.id).eq('is_sold', false)
...
export async function fetchFeaturedCars(limit = 6): Promise<Car[]> {
  const { data } = await supabase
    .from('cars').select('*').eq('is_sold', false).eq('is_hot_deal', true)
    .order('created_at', { ascending: false }).limit(limit)
  return (data as Car[]) || []
}
...
export async function updateCar(id: string, updates: Partial<Car>): Promise<Car> {
  const { data, error } = await supabase
    .from('cars').update({ ...updates, updated_at: new Date().toISOString() })
...
export async function uploadCarImage(file: File): Promise<string> {
  ...
  const { error } = await supabase.storage
    .from('car-images').upload(path, file, { cacheControl: '3600', upsert: false })
  if (error) throw error
...
export async function fetchBrands(): Promise<string[]> {
  const { data } = await supabase.from('cars').select('brand').eq('is_sold', false)
...
export async function updateSettings(updates: Partial<DealerSettings>): Promise<void> {
