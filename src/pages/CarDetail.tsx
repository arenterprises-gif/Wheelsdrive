    ...
    <div style={{ background:'#F8F9FC',minHeight:'100vh' }}>
      ...
      <div style={{ paddingTop:64,maxWidth:1100,margin:'0 auto',padding:'80px 24px 60px' }}>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:32 }}>
              ...
              <div style={{ display:'flex',gap:8,marginTop:10,overflowX:'auto',paddingBottom:4 }}>
                {imgs.map((url,i)=>(
                  <img key={url} src={url} alt="" onClick={()=>setPhoto(i)}
                    style={{ width:72,height:52,objectFit:'cover',borderRadius:8,cursor:'pointer',border:i===photo?'2px solid #0052CC':'2px solid transparent',flexShrink:0,transition:'border-color 0.2s' }}/>
                ))}
              </div>
            )}
          </div>
          ...
          {/* Info */}
          <div>
            <div style={{ color:'#0052CC',fontSize:12,fontWeight:700,letterSpacing:2,marginBottom:6 }}>{car.brand} · {car.segment}</div>
            ...
            <div style={{ display:'flex',alignItems:'baseline',gap:10,marginBottom:20 }}>
              <div style={{ fontFamily:'Nunito,sans-serif',fontWeight:800,fontSize:'clamp(24px,3vw,36px)',color:'#0052CC' }}>{fmt(car.price)}</div>
            ...
            <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:20 }}>
              {[
                ...
                <div key={i} style={{ background:'#F8F9FC',border:'1px solid #E8ECF0',borderRadius:10,padding:'10px 8px',textAlign:'center' }}>
                  <div style={{ color:'#0052CC',marginBottom:4,display:'flex',justifyContent:'center' }}>{icon as React.ReactNode}</div>
            ...
            <div style={{ display:'flex',gap:8,flexWrap:'wrap',marginBottom:20 }}>
              {car.is_hot_deal&&<span style={{ background:'#EBF2FF',color:'#0052CC',border:'1px solid #B3D1FF',fontSize:12,padding:'5px 12px',borderRadius:20,fontWeight:700 }}>🔥 Hot Deal</span>}
            ...
            {!car.is_sold?(
              ...
              <div style={{ background:'#F8F9FC',border:'1px solid #E8ECF0',borderRadius:10,padding:14,marginBottom:20,textAlign:'center',color:'#64748B' }}>
                ...
                <button onClick={()=>nav('/')} style={{ background:'none',border:'none',color:'#0052CC',cursor:'pointer',textDecoration:'underline',fontFamily:'Nunito,sans-serif' }}>Browse available cars →</button>
              </div>
            )}
        ...
        {car.features?.length>0&&(
          <div style={{ marginTop:28 }}>
            ...
            <div style={{ display:'flex',flexWrap:'wrap',gap:10 }}>
              {car.features.map(f=>(
                <span key={f} style={{ background:'#EBF2FF',color:'#0052CC',fontSize:13,padding:'6px 14px',borderRadius:20,border:'1px solid #B3D1FF',fontWeight:500 }}>✓ {f}</span>
              ))}
            </div>
          </div>
        )}
