    ...
    <div style={{ background:'#F8F9FC',minHeight:'100vh' }}>
      ...
      <div style={{ paddingTop:64,maxWidth:1100,margin:'0 auto',padding:'80px 24px 60px' }}>
        ...
        <div style={{ display:'flex',alignItems:'center',gap:8,marginBottom:24,color:'#94A3B8',fontSize:13 }}>
          <button onClick={()=>nav('/')} style={{ background:'none',border:'none',color:'#64748B',cursor:'pointer',display:'flex',alignItems:'center',gap:4,fontFamily:'DM Sans,sans-serif',fontSize:13 }}>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:32 }}>
              ...
              <div style={{ display:'flex',gap:8,marginTop:10,overflowX:'auto',paddingBottom:4 }}>
                {imgs.map((url,i)=>(
                  <img key={url} src={url} alt="" onClick={()=>setPhoto(i)}
                    style={{ width:72,height:52,objectFit:'cover',borderRadius:8,cursor:'pointer',border:i===photo?'2px solid #FF6B00':'2px solid transparent',flexShrink:0,transition:'border-color 0.2s' }}/>
                ))}
              </div>
            )}
          </div>
          ...
          {/* Info */}
          <div>
            <div style={{ color:'#FF6B00',fontSize:12,fontWeight:700,letterSpacing:2,marginBottom:6 }}>{car.brand} · {car.segment}</div>
            <h1 style={{ fontFamily:'Syne,sans-serif',fontWeight:800,fontSize:'clamp(20px,2.5vw,30px)',color:'#1A1A2E',marginBottom:4,lineHeight:1.2 }}>{car.title}</h1>
            <div style={{ color:'#94A3B8',fontSize:13,marginBottom:16 }}>{car.year} · {car.color}</div>
            <div style={{ display:'flex',alignItems:'baseline',gap:10,marginBottom:20 }}>
              <div style={{ fontFamily:'Syne,sans-serif',fontWeight:800,fontSize:'clamp(24px,3vw,36px)',color:'#FF6B00' }}>{fmt(car.price)}</div>
            ...
            <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:20 }}>
              {[
                ...
                <div key={i} style={{ background:'#F8F9FC',border:'1px solid #E8ECF0',borderRadius:10,padding:'10px 8px',textAlign:'center' }}>
                  <div style={{ color:'#FF6B00',marginBottom:4,display:'flex',justifyContent:'center' }}>{icon as React.ReactNode}</div>
            ...
            <div style={{ display:'flex',gap:8,flexWrap:'wrap',marginBottom:20 }}>
              {car.is_hot_deal&&<span style={{ background:'#FFF3E8',color:'#FF6B00',border:'1px solid #FFD5B0',fontSize:12,padding:'5px 12px',borderRadius:20,fontWeight:700 }}>🔥 Hot Deal</span>}
            ...
            {!car.is_sold?(
              ...
              <div style={{ background:'#F8F9FC',border:'1px solid #E8ECF0',borderRadius:10,padding:14,marginBottom:20,textAlign:'center',color:'#64748B' }}>
                ...
                <button onClick={()=>nav('/')} style={{ background:'none',border:'none',color:'#FF6B00',cursor:'pointer',textDecoration:'underline',fontFamily:'DM Sans,sans-serif' }}>Browse available cars →</button>
              </div>
            )}
        ...
        <div style={{ marginTop:32 }}>
          <h2 style={{ fontFamily:'Syne,sans-serif',fontWeight:800,fontSize:20,color:'#1A1A2E',marginBottom:16 }}>Specifications</h2>
        ...
        {car.features?.length>0&&(
          <div style={{ marginTop:28 }}>
            <h2 style={{ fontFamily:'Syne,sans-serif',fontWeight:800,fontSize:20,color:'#1A1A2E',marginBottom:16 }}>Features & Highlights</h2>
            <div style={{ display:'flex',flexWrap:'wrap',gap:10 }}>
              {car.features.map(f=>(
                <span key={f} style={{ background:'#FFF3E8',color:'#FF6B00',fontSize:13,padding:'6px 14px',borderRadius:20,border:'1px solid #FFD5B0',fontWeight:500 }}>✓ {f}</span>
              ))}
            </div>
          </div>
        )}
        ...
        {similar&&similar.length>0&&(
          <div style={{ marginTop:48 }}>
            <h2 style={{ fontFamily:'Syne,sans-serif',fontWeight:800,fontSize:22,color:'#1A1A2E',marginBottom:20 }}>Similar Cars</h2>
