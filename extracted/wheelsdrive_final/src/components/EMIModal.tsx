    ...
    <div style={{ position:'fixed',inset:0,background:'rgba(0,0,0,0.4)',zIndex:900,display:'flex',alignItems:'center',justifyContent:'center',padding:16 }} onClick={onClose}>
      <div className="fade-up" style={{ background:'#fff',borderRadius:20,padding:28,width:'100%',maxWidth:440,boxShadow:'0 20px 60px rgba(0,0,0,0.15)',maxHeight:'90vh',overflowY:'auto' }} onClick={e=>e.stopPropagation()}>
        <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20 }}>
          <div style={{ fontFamily:'Nunito,sans-serif',fontWeight:800,fontSize:20,color:'#1A1A2E' }}>EMI Calculator</div>
        ...
        <div style={{ background:'#FFF3E8',border:'1px solid #FFD5B0',borderRadius:12,padding:14,marginBottom:20,display:'flex',justifyContent:'space-between',alignItems:'center' }}>
          ...
          <div style={{ color:'#0052CC',fontWeight:800,fontSize:20,fontFamily:'Nunito,sans-serif' }}>{fmt(car.price)}</div>
        </div>
        {[
          ...
          <div key={label} style={{ marginBottom:18 }}>
            ...
            <input type="range" min={min} max={max} step={step} value={val} onChange={e=>set(Number(e.target.value))} style={{ width:'100%',accentColor:'#0052CC',cursor:'pointer',background:'none',border:'none',padding:0 }}/>
          </div>
        ))}
        <div style={{ background:'linear-gradient(135deg,#0052CC,#003D99)',borderRadius:14,padding:20,textAlign:'center',marginBottom:16,color:'#fff' }}>
          <div style={{ fontSize:13,opacity:0.85,marginBottom:4 }}>Monthly EMI</div>
          <div style={{ fontFamily:'Nunito,sans-serif',fontWeight:800,fontSize:40 }}>₹{emi.toLocaleString('en-IN')}</div>
