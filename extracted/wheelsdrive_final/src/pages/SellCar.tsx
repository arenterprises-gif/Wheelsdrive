    ...
    <div>
      ...
      {url ? (
        <div style={{ position:'relative',borderRadius:10,overflow:'hidden',border:'1px solid #E8ECF0' }}>
          ...
          <div style={{ position:'absolute',bottom:6,left:6,background:'rgba(0,82,204,0.9)',color:'#fff',fontSize:10,padding:'2px 8px',borderRadius:10,fontWeight:700 }}>✓ UPLOADED</div>
        </div>
      ) : (
        <div {...getRootProps()} style={{ border:`2px dashed ${isDragActive?'#0052CC':'#E3F2FD'}`,borderRadius:10,height:110,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',cursor:'pointer',background:isDragActive?'#EBF2FF':'#F8F9FC',transition:'all 0.2s',gap:6 }}>
          <input {...getInputProps()}/>
          {loading ? <Loader size={20} style={{ color:'#0052CC',animation:'spin 0.7s linear infinite' }}/> : <Upload size={20} style={{ color:'#CBD5E1' }}/>}
...
export default function SellCar() {
  ...
  const inp = useCallback((k: keyof ValuationForm) => ({
    ...
    onChange: (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => handleInputChange(e, k),
    style: { borderColor: errors[k] ? '#0052CC' : undefined }
  }), [form, errors, handleInputChange])
      ...
      <div style={{ paddingTop:64 }}>
        ...
        <div style={{ background:'linear-gradient(135deg,#0052CC,#003D99)',color:'#fff',padding:'40px 24px',textAlign:'center' }}>
          <div style={{ maxWidth:600,margin:'0 auto' }}>
            ...
            <h1 style={{ fontFamily:'Nunito,sans-serif',fontSize:'clamp(24px,4vw,38px)',fontWeight:800,marginBottom:10 }}>Get Instant AI Valuation</h1>
          ...
          <div style={{ maxWidth:700,margin:'0 auto',display:'flex',gap:0 }}>
            {STEPS.map((s,i)=>(
              <div key={s} style={{ flex:1,display:'flex',flexDirection:'column',alignItems:'center',position:'relative' }}>
                {i>0&&<div style={{ position:'absolute',top:14,left:'-50%',right:'50%',height:2,background:i<=step?'#0052CC':'#E3F2FD',transition:'background 0.3s' }}/>}
                <div style={{ width:28,height:28,borderRadius:'50%',background:i<step?'#0052CC':i===step?'#0052CC':'#F8F9FC',border:`2px solid ${i<=step?'#0052CC':'#E3F2FD'}`,display:'flex',alignItems:'center',justifyContent:'center',marginBottom:6,transition:'all 0.3s',zIndex:1 }}>
                  {i<step?<Check size={14} style={{ color:'#fff' }}/>:<span style={{ color:i===step?'#fff':'#94A3B8',fontSize:11,fontWeight:700 }}>{i+1}</span>}
                </div>
                <span style={{ color:i<=step?'#0052CC':'#94A3B8',fontSize:11,fontWeight:i===step?700:400,textAlign:'center' }}>{s}</span>
              </div>
            ))}
          </div>
        </div>
              ...
              <div>
                ...
                <div style={{ display:'flex',flexDirection:'column',gap:12,marginTop:20 }}>
                  {[
                    ...
                    <label key={String(key)} style={{ display:'flex',alignItems:'flex-start',gap:12,cursor:'pointer',padding:'14px 16px',background:'#F8F9FC',borderRadius:12,border:`1.5px solid ${form[key as keyof ValuationForm]?'#0052CC':'#E3F2FD'}`,transition:'all 0.2s' }}>
                      <input type="checkbox" checked={form[key as keyof ValuationForm] as boolean} onChange={e=>set(key as keyof ValuationForm,e.target.checked)} style={{ width:18,height:18,accentColor:'#0052CC',marginTop:1,cursor:'pointer' }}/>
                      <div>
            ...
            {step===2&&(
              <div>
                ...
                <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:14 }}>
                  ...
                  <div style={{ padding:12,background:'#EBF2FF',borderRadius:10,border:'1px dashed #FFB380',display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:4 }}>
                    <span style={{ fontSize:24 }}>💡</span>
                    <span style={{ color:'#003D99',fontSize:11,textAlign:'center',fontWeight:500 }}>Good lighting = Better valuation accuracy</span>
                  </div>
                </div>
              </div>
            )}
