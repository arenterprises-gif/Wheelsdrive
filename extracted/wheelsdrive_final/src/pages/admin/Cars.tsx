    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ color: '#9CA3AF', fontSize: 13, fontWeight: 500 }}>
            <span style={{ fontWeight: 700, color: '#0052CC' }}>{total}</span> cars in inventory
          </p>
        </div>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <button
            ...
            style={{
              ...
              border: '1.5px solid #0052CC', color: '#0052CC',
              background: '#EBF2FF', borderRadius: 10, padding: '10px 18px',
          ...
          <button onClick={() => nav('/admin/cars/add')}
            ...
            onMouseEnter={e => {
              ...
              (e.currentTarget as HTMLButtonElement).style.boxShadow = '0 6px 18px rgba(0,82,204,0.4)';
            }}
            onMouseLeave={e => {
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 18, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#6B7280', fontSize: 13, cursor: 'pointer', fontWeight: 600, background: '#FFFFFF', border: '1.5px solid #E5E7EB', borderRadius: 10, padding: '10px 14px' }}>
          <input type="checkbox" checked={showSold} onChange={e => setShowSold(e.target.checked)} style={{ accentColor: '#0052CC', cursor: 'pointer' }} />
          Show Sold
        </label>
      ...
      {isLoading ? (
        ...
        <div style={{
          ...
          {!search && (
            <button onClick={() => nav('/admin/cars/add')}
              style={{ marginTop: 16, background: '#0052CC', color: '#fff', border: 'none', borderRadius: 10, padding: '10px 24px', cursor: 'pointer', fontWeight: 700, fontSize: 14, fontFamily: 'Nunito,sans-serif' }}>
              + Add First Car
            </button>
          )}
        </div>
          ...
          <div style={{ background: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: 16, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
            ...
            {cars.map((car, i) => (
              <div key={car.id}
                ...
                <div style={{ fontFamily: 'Nunito,sans-serif', fontWeight: 800, fontSize: 14, color: '#0052CC' }}>{fmt(car.price)}</div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  ...
                  {car.is_hot_deal && <span style={{ background: '#EBF2FF', color: '#0052CC', fontSize: 10, padding: '2px 6px', borderRadius: 6, textAlign: 'center', fontWeight: 700 }}>🔥 HOT</span>}
                ...
                <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                  ...
                  <button onClick={() => toggle.mutate({ id: car.id, field: 'is_hot_deal', val: !car.is_hot_deal })}
                    style={{ background: car.is_hot_deal ? '#EBF2FF' : '#F3F4F6', border: `1px solid ${car.is_hot_deal ? '#B3D1FF' : '#E5E7EB'}`, color: car.is_hot_deal ? '#0052CC' : '#374151', borderRadius: 7, padding: '5px 9px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 3, fontSize: 11, fontWeight: 600, transition: 'all 0.15s' }}>
                    <Flame size={11} />
                  </button>
              ...
              {Array.from({ length: Math.min(totalPages, 10) }, (_, i) => i + 1).map(p => (
                <button key={p} onClick={() => setPage(p)}
                  style={{
                    ...
                    border: `1.5px solid ${p === page ? '#0052CC' : '#E5E7EB'}`,
