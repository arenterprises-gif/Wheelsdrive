...
export default function AdminDashboard() {
  ...
  const StatCard = ({
    icon, label, value, sub, color = '#0052CC', loading,
  }: {
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
        ...
        <button onClick={() => nav('/admin/cars/add')}
          ...
          onMouseEnter={e => {
            ...
            (e.currentTarget as HTMLButtonElement).style.boxShadow = '0 6px 20px rgba(0,82,204,0.4)';
          }}
          onMouseLeave={e => {
            (e.currentTarget as HTMLButtonElement).style.transform = 'translateY(0)';
            (e.currentTarget as HTMLButtonElement).style.boxShadow = '0 4px 14px rgba(0,82,204,0.3)';
          }}>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(150px,1fr))', gap: 12 }}>
          {[
            { label: 'Add New Car', icon: '🚗', action: () => nav('/admin/cars/add'), color: '#0052CC' },
      ...
      <div style={{
        ...
        {recent.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '32px 0', color: '#9CA3AF' }}>
            ...
            <button onClick={() => nav('/admin/cars/add')}
              style={{ background: 'none', border: 'none', color: '#0052CC', cursor: 'pointer', textDecoration: 'underline', fontSize: 13, marginTop: 4 }}>
              Add your first car
            </button>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {recent.map((car, i) => (
              <div key={car.id} style={{
                ...
                <div style={{ fontFamily: 'Nunito,sans-serif', fontWeight: 800, fontSize: 15, color: '#0052CC', flexShrink: 0 }}>{fmt(car.price)}</div>
                <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                  {car.is_sold && (
                    <span style={{ background: '#F3F4F6', color: '#9CA3AF', fontSize: 10, padding: '2px 8px', borderRadius: 8, fontWeight: 700 }}>SOLD</span>
                  )}
                  {car.is_hot_deal && (
                    <span style={{ background: '#EBF2FF', color: '#0052CC', fontSize: 10, padding: '2px 8px', borderRadius: 8, fontWeight: 700, border: '1px solid #B3D1FF' }}>HOT</span>
                  )}
                  {car.is_featured && (
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
            <h2 style={{ fontFamily: 'Nunito,sans-serif', fontWeight: 800, fontSize: 17, color: '#111827', display: 'flex', alignItems: 'center', gap: 10 }}>
              Recent Inquiries
              {unread > 0 && (
                <span style={{ background: '#0052CC', color: '#fff', fontSize: 11, padding: '2px 8px', borderRadius: 20, fontWeight: 700 }}>
                  {unread} new
                </span>
              )}
            </h2>
            <button onClick={() => nav('/admin/inquiries')}
              style={{ background: 'none', border: 'none', color: '#0052CC', cursor: 'pointer', fontSize: 13, fontWeight: 700, fontFamily: 'Nunito,sans-serif' }}>
              View All →
            </button>
          </div>
          {inquiries.slice(0, 4).map((inq, i) => (
            <div key={inq.id} style={{
              ...
              <div style={{
                ...
                background: inq.is_read ? '#F3F4F6' : '#EBF2FF',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: 16, flexShrink: 0,
                border: inq.is_read ? 'none' : '1.5px solid #B3D1FF',
              }}>💬</div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ color: '#111827', fontWeight: 700, fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>
                  {inq.customer_name || 'Customer'}
                  {!inq.is_read && <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#0052CC', display: 'inline-block' }} />}
                </div>
