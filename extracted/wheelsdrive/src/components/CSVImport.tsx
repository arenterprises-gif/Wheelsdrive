...
export const CSVImport = ({ onComplete }: { onComplete: () => void }) => {
  ...
  if (phase === 'upload') return (
    <div style={{ display:'flex',flexDirection:'column',gap:16 }}>
      <div
        ...
        onMouseEnter={e=>{const d=e.currentTarget as HTMLDivElement;d.style.borderColor='var(--color-primary)';d.style.background='var(--color-primary-bg)'}}
        onMouseLeave={e=>{const d=e.currentTarget as HTMLDivElement;d.style.borderColor='var(--color-border)';d.style.background=''}}
      >
        <Upload size={32} style={{ color:'var(--color-text-muted)',margin:'0 auto 12px' }}/>
        <p style={{ color:'var(--color-text-secondary)',marginBottom:4,fontWeight:600 }}>CSV file choose karo</p>
        <p style={{ color:'var(--color-text-muted)',fontSize:12 }}>Max 500 cars ek baar mein</p>
        <input ref={fileRef} type="file" accept=".csv" onChange={handleFile} style={{ display:'none' }}/>
      </div>
      <button onClick={downloadTemplate} style={{ display:'flex',alignItems:'center',gap:8,color:'var(--color-primary)',background:'none',border:'none',cursor:'pointer',fontSize:13,fontWeight:600 }}>
    ...
    <div style={{ display:'flex',flexDirection:'column',gap:12 }}>
      <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:8 }}>
        <div style={{ display:'flex',gap:16 }}>
          <span style={{ color:'var(--color-success)',fontSize:13,display:'flex',alignItems:'center',gap:4 }}><CheckCircle size={14}/> {validCount} ready</span>
          {errorCount>0&&<span style={{ color:'var(--color-error)',fontSize:13,display:'flex',alignItems:'center',gap:4 }}><XCircle size={14}/> {errorCount} errors</span>}
        </div>
      ...
      {errorCount>0&&(
        <div style={{ background:'var(--color-error-bg)',border:'1px solid #FECACA',borderRadius:10,padding:'10px 14px' }}>
          <p style={{ fontSize:13,fontWeight:700,color:'var(--color-error)',marginBottom:6 }}>Fix karo CSV mein:</p>
          {rows.filter(r=>r.status==='error').map(r=>(
            <p key={r.row} style={{ fontSize:12,color:'var(--color-error)' }}>Row {r.row}: {r.error}</p>
          ))}
        </div>
      )}
      <div style={{ overflowX:'auto',maxHeight:260,border:'1px solid var(--color-border)',borderRadius:10 }}>
        <table style={{ width:'100%',borderCollapse:'collapse',fontSize:13 }}>
          <thead style={{ background:'var(--color-surface-2)',position:'sticky',top:0 }}>
            <tr>{['Row','Car','Price','Status'].map(h=><th key={h} style={{ textAlign:'left',padding:'8px 12px',color:'var(--color-text-secondary)',fontWeight:600 }}>{h}</th>)}</tr>
          </thead>
          <tbody>
            {rows.map(r=>(
              <tr key={r.row} style={{ borderTop:'1px solid var(--color-border-light)' }}>
                <td style={{ padding:'8px 12px',color:'var(--color-text-muted)' }}>{r.row}</td>
                ...
                <td style={{ padding:'8px 12px' }}>
                  {r.status==='error'?<span style={{ color:'var(--color-error)',fontSize:11 }}>{r.error}</span>
                  :r.status==='success'?<span style={{ color:'var(--color-success)',fontSize:11 }}>✓ Imported</span>
                  :<span style={{ color:'var(--color-text-muted)',fontSize:11 }}>Ready</span>}
                </td>
              </tr>
            ))}
          </tbody>
  ...
  return (
    <div style={{ textAlign:'center',padding:'32px 0' }}>
      {importing ? (
        <>
          <div style={{ width:'100%',background:'var(--color-border)',borderRadius:99,height:6,marginBottom:12 }}>
            <div style={{ width:`${progress}%`,background:'var(--color-primary)',borderRadius:99,height:'100%',transition:'width 0.3s' }}/>
          </div>
          <p style={{ color:'var(--color-text-secondary)' }}>{progress}% complete...</p>
        </>
      ) : (
        <>
          <CheckCircle size={48} style={{ color:'var(--color-success)',margin:'0 auto 12px' }}/>
          <p style={{ fontWeight:700,color:'var(--color-text-primary)' }}>Import complete!</p>
