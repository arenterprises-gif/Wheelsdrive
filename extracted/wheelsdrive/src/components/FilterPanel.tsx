    ...
    <div style={{ marginBottom: 22 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14, flexWrap: 'wrap' }}>
        <button
          ...
          style={{
            background: open || activeCount > 0 ? '#E8520A' : '#FFFFFF',
            color: open || activeCount > 0 ? '#fff' : '#6B7280',
            border: `1.5px solid ${open || activeCount > 0 ? '#E8520A' : '#E5E7EB'}`,
            borderRadius: 10, padding: '9px 16px', cursor: 'pointer',
            display: 'flex', alignItems: 'center', gap: 8,
            fontWeight: 700, fontSize: 13, fontFamily: 'Nunito,sans-serif',
            transition: 'all 0.2s',
            boxShadow: open || activeCount > 0 ? '0 3px 10px rgba(232,82,10,0.25)' : 'none',
          }}
          onMouseEnter={e => {
          ...
          Filters
          {activeCount > 0 && (
            <span style={{ background: '#fff', color: '#E8520A', borderRadius: '50%', width: 20, height: 20, fontSize: 11, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900 }}>
              {activeCount}
            </span>
          )}
        ...
        <span style={{ color: '#9CA3AF', fontSize: 13, marginLeft: 'auto', fontWeight: 600 }}>
          <span style={{ color: '#E8520A', fontWeight: 800 }}>{total}</span> {total === 1 ? 'car' : 'cars'} found
        </span>
      </div>
      ...
      {open && (
        ...
        >
            ...
            <div key={key} style={{ position: 'relative' }}>
              ...
              <div style={{ position: 'relative' }}>
                <select
                  style={{
                    ...sel,
                    borderColor: (filters as Record<string, unknown>)[key] ? '#E8520A' : '#E5E7EB',
                    color: (filters as Record<string, unknown>)[key] ? '#E8520A' : '#374151',
                    background: (filters as Record<string, unknown>)[key] ? '#FFF5F0' : '#FFFFFF',
                    paddingRight: 28,
                  }}
                  value={((filters as Record<string, unknown>)[key] as string) || ''}
                  onChange={e => set(key, e.target.value)}
                  onFocus={e => (e.currentTarget as HTMLSelectElement).style.borderColor = '#E8520A'}
                  onBlur={e => (e.currentTarget as HTMLSelectElement).style.borderColor = (filters as Record<string, unknown>)[key] ? '#E8520A' : '#E5E7EB'}
                >
          ...
          <div style={{ position: 'relative' }}>
            ...
            <div style={{ position: 'relative' }}>
              <select
                style={{ ...sel, paddingRight: 28, borderColor: filters.max_price ? '#E8520A' : '#E5E7EB', background: filters.max_price ? '#FFF5F0' : '#FFFFFF', color: filters.max_price ? '#E8520A' : '#374151' }}
          ...
          <div style={{ position: 'relative' }}>
            ...
            <div style={{ position: 'relative' }}>
              <select
                style={{ ...sel, paddingRight: 28, borderColor: filters.min_year ? '#E8520A' : '#E5E7EB', background: filters.min_year ? '#FFF5F0' : '#FFFFFF', color: filters.min_year ? '#E8520A' : '#374151' }}
          ...
          <div style={{ display: 'flex', alignItems: 'center', paddingTop: 18 }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', color: '#374151', fontSize: 13, fontFamily: 'Nunito,sans-serif', fontWeight: 600 }}>
              <input
                ...
                style={{ width: 16, height: 16, accentColor: '#E8520A', cursor: 'pointer' }}
              />
              🔥 Hot Deals
            </label>
          </div>
