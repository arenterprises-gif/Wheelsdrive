    ...
    <div>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 4 }}>
          ...
          {unread > 0 && (
            <span style={{ background: '#E8520A', color: '#fff', fontSize: 12, padding: '3px 12px', borderRadius: 20, fontWeight: 700, fontFamily: 'Nunito,sans-serif' }}>
              {unread} new
            </span>
          )}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {inquiries.map(inq => (
            <div
              ...
              style={{
                background: '#FFFFFF',
                border: `1.5px solid ${!inq.is_read ? '#FDDCCE' : '#E5E7EB'}`,
                borderRadius: 16,
                padding: '18px 20px',
                boxShadow: !inq.is_read ? '0 2px 12px rgba(232,82,10,0.08)' : '0 1px 4px rgba(0,0,0,0.04)',
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8, flexWrap: 'wrap' }}>
                    {!inq.is_read && (
                      <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#E8520A', flexShrink: 0, display: 'inline-block' }} />
                    )}
                  ...
                  {/* Car reference */}
                  {inq.car_title && (
                    <div style={{ color: '#E8520A', fontSize: 13, marginBottom: 8, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6 }}>
                      🚗 Re: {inq.car_title}
                    </div>
                  )}
