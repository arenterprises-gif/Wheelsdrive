  ...
  return (
    <div>
      ...
      {url ? (
        ...
        <div {...getRootProps()} style={{ border:`2px dashed ${isDragActive?'#E8520A':'#E8ECF0'}`,borderRadius:10,height:110,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',cursor:'pointer',background:isDragActive?'#FFF3E8':'#F8F9FC',transition:'all 0.2s',gap:6 }}>
          <input {...getInputProps()}/>
          {loading ? <Loader size={20} style={{ color:'#E8520A',animation:'spin 0.7s linear infinite' }}/> : <Upload size={20} style={{ color:'#CBD5E1' }}/>}
...
export default function SellCar() {
  ...
  const inp = useCallback((k: keyof ValuationForm) => ({
    ...
    style: { borderColor: errors[k] ? '#E8520A' : undefined }
  ...
  return (
    <div style={{ background:'#F8F9FC',minHeight:'100vh' }}>
      ...
      <div style={{ paddingTop:64 }}>
        {/* Hero */}
        <div style={{ background:'linear-gradient(135deg,#E8520A,#E55A00)',color:'#fff',padding:'40px 24px',textAlign:'center' }}>
          ...
          <div style={{ maxWidth:700,margin:'0 auto',display:'flex',gap:0 }}>
            {STEPS.map((s,i)=>(
              <div key={s} style={{ flex:1,display:'flex',flexDirection:'column',alignItems:'center',position:'relative' }}>
                {i>0&&<div style={{ position:'absolute',top:14,left:'-50%',right:'50%',height:2,background:i<=step?'#E8520A':'#E8ECF0',transition:'background 0.3s' }}/>}
                <div style={{ width:28,height:28,borderRadius:'50%',background:i<step?'#E8520A':i===step?'#E8520A':'#F8F9FC',border:`2px solid ${i<=step?'#E8520A':'#E8ECF0'}`,display:'flex',alignItems:'center',justifyContent:'center',marginBottom:6,transition:'all 0.3s',zIndex:1 }}>
                  {i<step?<Check size={14} style={{ color:'#fff' }}/>:<span style={{ color:i===step?'#fff':'#94A3B8',fontSize:11,fontWeight:700 }}>{i+1}</span>}
                </div>
                <span style={{ color:i<=step?'#E8520A':'#94A3B8',fontSize:11,fontWeight:i===step?700:400,textAlign:'center' }}>{s}</span>
              </div>
            ))}
          </div>
        </div>
              ...
              <div>
                ...
                <div style={{ display:'flex',flexDirection:'column',gap:12,marginTop:20 }}>
                  {[
                    ...
                    <label key={String(key)} style={{ display:'flex',alignItems:'flex-start',gap:12,cursor:'pointer',padding:'14px 16px',background:'#F8F9FC',borderRadius:12,border:`1.5px solid ${form[key as keyof ValuationForm]?'#E8520A':'#E8ECF0'}`,transition:'all 0.2s' }}>
                      <input type="checkbox" checked={form[key as keyof ValuationForm] as boolean} onChange={e=>set(key as keyof ValuationForm,e.target.checked)} style={{ width:18,height:18,accentColor:'#E8520A',marginTop:1,cursor:'pointer' }}/>
                      <div>
