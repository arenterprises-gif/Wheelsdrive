...
export default function Home() {
  ...
  return (
    <div style={{ background: '#F4F5F7', minHeight: '100vh' }}>
      ...
      {/* ── Hero ─────────────────────────────────────── */}
      <div style={{ paddingTop: 64, background: 'linear-gradient(160deg,#FFFFFF 0%,#FFF8F5 60%,#FFF3EE 100%)', borderBottom: '1px solid #F3F4F6' }}>
        <div style={{ maxWidth: 1260, margin: '0 auto', padding: 'clamp(40px,6vw,80px) 24px clamp(36px,5vw,72px)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'clamp(32px,5vw,72px)', alignItems: 'center' }}>
          <div className="fade-up">
            <div style={{ background: 'linear-gradient(135deg,#FFF5F0,#FFE8DC)', border: '1.5px solid #FDDCCE', display: 'inline-flex', alignItems: 'center', gap: 6, borderRadius: 24, padding: '7px 16px', fontSize: 11, color: '#E8520A', fontWeight: 800, letterSpacing: 1.2, marginBottom: 24, fontFamily: 'Nunito,sans-serif' }}>
            ...
            <h1 style={{ fontFamily: 'Nunito,sans-serif', fontSize: 'clamp(30px,4.5vw,58px)', fontWeight: 900, lineHeight: 1.12, marginBottom: 20, color: '#111827', letterSpacing: -1 }}>
              Buy & Sell<br />
              <span style={{ background: 'linear-gradient(135deg,#E8520A,#FF6B2B)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                Used Cars
              </span>
              <br />with Confidence
            </h1>
            ...
            <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
              <button
                ...
                style={{
                  ...
                  boxShadow: '0 8px 24px rgba(232,82,10,0.32)',
                  transition: 'all 0.25s', fontFamily: 'Nunito,sans-serif',
                }}
                onMouseEnter={e => {
                  (e.currentTarget as HTMLButtonElement).style.transform = 'translateY(-2px)';
                  (e.currentTarget as HTMLButtonElement).style.boxShadow = '0 12px 32px rgba(232,82,10,0.42)';
                }}
                onMouseLeave={e => {
                  (e.currentTarget as HTMLButtonElement).style.transform = 'translateY(0)';
                  (e.currentTarget as HTMLButtonElement).style.boxShadow = '0 8px 24px rgba(232,82,10,0.32)';
                }}>
                Browse Cars <ArrowRight size={16} />
              </button>
              <Link to="/sell"
                style={{
                  background: '#fff', border: '2px solid #E8520A', color: '#E8520A',
                  ...
                  boxShadow: '0 4px 12px rgba(232,82,10,0.12)',
                }}
                onMouseEnter={e => {
                  (e.currentTarget as HTMLAnchorElement).style.background = '#FFF5F0';
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }} className="slide-in">
            {[
              { icon: '🚗', label: 'Cars Available', value: stats?.available ?? '…', color: '#E8520A', bg: '#FFF5F0', border: '#FDDCCE' },
      ...
      <div style={{ background: '#FFFFFF', borderBottom: '1px solid #F3F4F6', padding: '16px 24px', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
        <div style={{ maxWidth: 680, margin: '0 auto', display: 'flex', background: '#F9FAFB', borderRadius: 12, border: '1.5px solid #E5E7EB', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
          ...
          <button
            ...
            onMouseLeave={e => (e.currentTarget as HTMLButtonElement).style.background = 'linear-gradient(135deg,#E8520A,#FF6B2B)'}>
            Search
          </button>
        </div>
      </div>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', justifyContent: 'center', gap: 'clamp(16px,4vw,56px)', flexWrap: 'wrap' }}>
          {[
            ...
            <div key={String(label)} style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'rgba(255,255,255,0.7)', fontSize: 13 }}>
              <span style={{ color: '#E8520A' }}>{icon as React.ReactNode}</span>{String(label)}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1200, margin: '40px auto 0', padding: '0 24px' }}>
        <div style={{
          ...
          boxShadow: '0 8px 32px rgba(232,82,10,0.25)',
        }}>
          <div>
        ...
        <div style={{ maxWidth: 1200, margin: '48px auto 0', padding: '0 24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            ...
            <button
              ...
              style={{ background: '#FFF5F0', border: '1.5px solid #FDDCCE', color: '#E8520A', borderRadius: 8, padding: '7px 14px', cursor: 'pointer', fontSize: 13, fontFamily: 'Nunito,sans-serif', display: 'flex', alignItems: 'center', gap: 6, fontWeight: 600, transition: 'all 0.18s' }}
              ...
              onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = '#FFF5F0'; (e.currentTarget as HTMLButtonElement).style.color = '#E8520A'; }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 8, marginTop: 40 }}>
                ...
                {Array.from({ length: Math.min(totalPages, 7) }, (_, i) => i + 1).map(p => (
                  <button key={p} onClick={() => setPage(p)}
                    style={{ background: p === page ? '#E8520A' : '#fff', border: `1.5px solid ${p === page ? '#E8520A' : '#E5E7EB'}`, color: p === page ? '#fff' : '#374151', borderRadius: 8, padding: '8px 14px', cursor: 'pointer', fontWeight: p === page ? 800 : 500, minWidth: 40, boxShadow: '0 1px 3px rgba(0,0,0,0.06)', fontFamily: 'Nunito,sans-serif', transition: 'all 0.15s' }}>
                    {p}
                  </button>
                ))}
                <button
